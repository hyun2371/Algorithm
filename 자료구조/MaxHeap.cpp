...
void MaxHeap::insert(int key) {
	...
	//새 노드가 최대힙 성질을 만족하도록 올림
	while (i != 1 && key > getParent(i).getKey()) { //부모보다 키값이 큰 경우 반복
...
HeapNode MaxHeap::remove() {
	...
	while (child <= size) { //힙 트리 크기 내에서
		...
		if (child < size  && getLeft(parent).getKey() < getRight(parent).getKey())
			child++; //오른쪽 자식이 크면 오른쪽으로 감
		if (last.getKey() >= node[child].getKey()) 
			break; //자식 노드보다 크면 오케이

		node[parent] = node[child];  //자식 노드보다 작으면 교환
